...
export class InvestigationController {
  ...
  create = async (request: Request, response: Response, next: NextFunction) => { try { response.status(201).json(await this.service.create(request.auth!, request.body)); } catch (error) { next(error); } };
  createForProject = async (request: Request, response: Response, next: NextFunction) => { try { response.status(201).json(await this.service.create(request.auth!, request.body, String(request.params.projectId))); } catch (error) { next(error); } };
  preflightForProject = async (request: Request, response: Response, next: NextFunction) => { try { response.json(await this.service.preflight(request.auth!, request.body, String(request.params.projectId))); } catch (error) { next(error); } };
