...
import { PanelState, StatusBadge } from '../runtime/runtime-components.js';
...
export function RepairVerificationDetailPage() {
  ...
  if (detail.error || !detail.data) return <PanelState title="Repair Verification unavailable" retry={() => void detail.refetch()}>{detail.error instanceof Error ? detail.error.message : 'TaskOS could not load this verification.'}</PanelState>;
  ...
  return <section className="mx-auto max-w-3xl"><PageHeading eyebrow="Repair verification" title="Repair Verification result" description="The result is derived from the prepared minimal reproduction, original controls, and bounded regression Worlds." action={<Link className="rounded-lg border border-slate-700 px-4 py-2 text-sm" to={`/investigations/${investigationId}/findings/${findingId}`}>Back to Finding</Link>} />
    <div className="card mt-6"><div className="flex flex-wrap items-center gap-3"><StatusBadge tone={item.executionStatus === 'COMPLETED' ? 'green' : item.executionStatus === 'FAILED' ? 'red' : 'slate'}>{item.executionStatus}</StatusBadge>{item.verificationResult ? <strong>{item.verificationResult.replaceAll('_', ' ')}</strong> : <span>Awaiting result</span>}</div>{item.comparison ? <p className="mt-4 text-sm text-slate-300">{item.comparison.reason}</p> : null}{item.failure ? <p className="mt-4 text-sm text-red-300">{item.failure.message}</p> : null}{item.executionStatus === 'QUEUED' && editable ? <button className="mt-5 rounded-lg border border-red-400 px-4 py-2 text-sm text-red-200" disabled={cancel.isPending} onClick={() => cancel.mutate()}>Cancel queued verification</button> : null}</div>
