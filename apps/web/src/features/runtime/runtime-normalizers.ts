...
export type PlannerProvenanceViewModel = {
  requested: string;
  effective: string;
  status: string;
  model: string | null;
...
export function providerFromPlan(plan: ExperimentPlanResponse | null | undefined): PlannerProvenanceViewModel {
  ...
  const durationMs = typeof metadata.generationDurationMs === 'number' && Number.isFinite(metadata.generationDurationMs) ? metadata.generationDurationMs : null;
  const fallbackUsed = effective === 'FALLBACK' || status === 'FALLBACK_USED';
  const schemaPassed = typeof plan?.schemaPassed === 'boolean' ? plan.schemaPassed : ['ACCEPTED', 'PARTIALLY_ACCEPTED', 'FALLBACK_USED'].includes(status);
  const rejectedWorldCount = typeof metadata.rejectedWorldCount === 'number' && Number.isFinite(metadata.rejectedWorldCount) ? metadata.rejectedWorldCount : 0;
  const safetyPassed = typeof plan?.safetyPassed === 'boolean' ? plan.safetyPassed : schemaPassed && (fallbackUsed || rejectedWorldCount === 0);
  const kimiVerified = (effective === 'KIMI' || effective === 'AIAND') && !fallbackUsed && status === 'ACCEPTED' && schemaPassed && safetyPassed;
  const sourceLabel = fallbackUsed ? 'Generated by deterministic fallback' : effective === 'AIAND' ? 'Generated by Kimi via ai&' : effective === 'KIMI' ? 'Generated by Kimi' : `Generated by ${plannerProviderLabel(effective)}`;
  const badgeLabel = kimiVerified ? 'Kimi Verified' : fallbackUsed ? 'Fallback plan' : status === 'FAILED' ? 'Planning failed' : effective === 'Not recorded' ? 'Plan provenance unknown' : plannerProviderLabel(effective);
  return { requested, effective, status, model, fallbackUsed, fallbackReason: fallbackReason(plan), generatedAt, durationMs, schemaPassed, safetyPassed, kimiVerified, sourceLabel, badgeLabel };
}

export function plannerProviderLabel(provider: string): string {
  if (provider === 'AIAND') return 'Kimi via ai&';
