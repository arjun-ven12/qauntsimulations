...
describe('runtime world normalization', () => {
  it('normalizes Kimi planner provenance independently from fallback source', () => {
    ...
    expect(providerFromPlan({ schemaPassed: true, safetyPassed: true, plannerMetadata: { requestedProvider: 'AIAND', effectiveProvider: 'AIAND', plannerStatus: 'ACCEPTED', model: 'moonshotai/kimi-k2.7-code' } } as never)).toMatchObject({ requested: 'AIAND', effective: 'AIAND', model: 'moonshotai/kimi-k2.7-code', fallbackUsed: false, kimiVerified: true, sourceLabel: 'Generated by Kimi via ai&' });
    ...
    expect(plannerProviderLabel('AIAND')).toBe('Kimi via ai&');
